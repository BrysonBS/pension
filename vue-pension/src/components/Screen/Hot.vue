...
export default {
  ...
  mounted() {
    // 在组件创建完成之后，进行回调函数的注册
    this.$socket.registerCallBack("screenOrderStatusHandler", this.getData);
    this.initChart();
    // this.getData();
    this.$socket.send({
      operate: "screenOrderStatusHandler"
    });
  ...
  destroyed() {
    ...
    this.$socket.unRegisterCallBack("screenOrderStatusHandler");
  },
  computed: {
