...
<script>
import { getProvinceMapInfo } from "@/utils/screen/map_utils";
import { mapState } from "vuex";
import { getThemeValue } from "@/utils/screen/theme_utils";
const chinaJson = require('@/assets/screen/map/china.json')
export default {
  data() {
    return {
      myChart: null,
  ...
  mounted() {
    // 在组件创建完成之后，进行回调函数的注册
    this.$socket.registerCallBack("screenMapHandler", this.getData);
    this.initChart();
    // this.getData();
    //console.log("send")
    this.$socket.send({
      operate: "screenMapHandler",
      test: this.test,
    })
  ...
  destroyed() {
    ...
    this.$socket.unRegisterCallBack("screenMapHandler");
  },
  computed: {
    ...mapState("screen",["theme","test"]),
    btnStyle() {
      return {
...
/*            const { data: ret } = await axios.get(
            ...
            const ret = require('@/assets/screen/map/province/' +
              provinceInfo.path.replace(/\/.*\//g,''));
