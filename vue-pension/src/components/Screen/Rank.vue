...
export default {
  ...
  mounted() {
    // 在组件创建完成之后，进行回调函数的注册
    this.$socket.registerCallBack("screenMonthOfOrderHandler", this.getData);
    this.initChart();
    // this.getData();
    this.$socket.send({
      operate: "screenMonthOfOrderHandler"
    })
  ...
  destroyed() {
    ...
    this.$socket.unRegisterCallBack("screenMonthOfOrderHandler");
  },
  computed: {
