...
export default {
  ...
  mounted() {
    // 在组件创建完成之后，进行回调函数的注册
    this.$socket.registerCallBack("screenMonthOfOrderByDeptHandler", this.getData);
    this.initChart();
    // this.getData();
    // 发送数据给服务端，告诉服务端，前端现在需要数据
    this.$socket.send({
      operate: "screenMonthOfOrderByDeptHandler"
    })
  ...
  destroyed() {
    ...
    // 在组件销毁时，进行回调函数取消
    this.$socket.unRegisterCallBack("screenMonthOfOrderByDeptHandler");
  },
  computed: {
